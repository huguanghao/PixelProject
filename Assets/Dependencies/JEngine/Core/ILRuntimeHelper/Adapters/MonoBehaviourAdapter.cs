...
public class MonoBehaviourAdapter : CrossBindingAdaptor
{
    ...
    public class Adaptor : MonoBehaviour, CrossBindingAdaptorType
    {
        ...
        public void Reset()
        {
            ...
            /// 添加关键帧函数绑定 - 2022/6/29 dk
            _mKeyPointMethodGot = false;
            instance = null;
            appdomain = null;
            _destoryed = false;
            awaked = false;
        ...
        public void Awake()
        {
            ...
            if (instance != null)
            {
                ...
                LifeCycleMgr.Instance.AddTask(instance, () =>
                {
                    ...
                    if ((methodInfo = GetMethodInfo(type, "FixedUpdate")) != null)
                        LifeCycleMgr.Instance.AddFixedUpdateItem(instance, methodInfo,
                            gameObject, () => enabled);
                    if ((methodInfo = GetMethodInfo(type, "Update")) != null)
                        LifeCycleMgr.Instance.AddUpdateItem(instance, methodInfo,
                            gameObject, () => enabled);
                    if ((methodInfo = GetMethodInfo(type, "LateUpdate")) != null)
                        LifeCycleMgr.Instance.AddLateUpdateItem(instance, methodInfo,
                            gameObject, () => enabled);
        ...
        /// 添加关键帧函数绑定 - 2022/6/29 dk
        IMethod _mKeyPointMethod;
        bool _mKeyPointMethodGot;
        void keypoint(string parma)
        {
            if (instance != null && awaked)
            {
                if (!_mKeyPointMethodGot)
                {
                    _mKeyPointMethod = instance.Type.GetMethod("keypoint", 1);
                    _mKeyPointMethodGot = true;
                }

                if (_mKeyPointMethod != null && awaked)
                {
                    appdomain.Invoke(_mKeyPointMethod, instance, parma);
                }
            }
        }
    }
