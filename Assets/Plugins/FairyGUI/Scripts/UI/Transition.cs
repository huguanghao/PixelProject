...
namespace FairyGUI
{
    ...
    public class Transition : ITweenListener
    {
        ...
        public int GetLabelIndex(string label)
        {
            ...
            {
                TransitionItem item = _items[i];
                if (item.label == label || (item.tweenConfig != null && item.tweenConfig.endLabel == label))
                    return i;
            }
            return -1;
        }

        public void SetValueFloat2(int labelIndex, bool isTweenEnd, float x, float y)
        {
            TransitionItem item = _items[labelIndex];
            if (item.tweenConfig != null)
            {
                TValue value = isTweenEnd ? item.tweenConfig.endValue : item.tweenConfig.startValue;
                switch (item.type)
                {
