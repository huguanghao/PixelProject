...
namespace FairyGUI
{
    ...
    public class StageCamera : MonoBehaviour
    {
        ...
        {
            var list = Camera.allCameras;
            for (int i = 0; i < list.Length; i++)
                list[i].pixelRect = pixelRect;
        }
        ...
        void OnScreenSizeChanged(int newWidth, int newHeight)
        {
            ...
            #region 强制游戏画面宽高比为设计比率比值
            UIContentScaler scaler0 = GameObject.FindObjectOfType<UIContentScaler>();
            if (scaler0 != null && scaler0.scaleMode == UIContentScaler.ScaleMode.ScaleWithScreenSize)
            {
                float designRatio = scaler0.designResolutionX * 1f / scaler0.designResolutionY;
                if (screenWidth >= screenHeight * designRatio)
                {
                    screenWidth = Mathf.CeilToInt(newHeight * designRatio);
                    pixelRect = new Rect((newWidth - screenWidth) / 2f, 0, screenWidth, screenHeight);
                }
                else
                {
                    screenHeight = Mathf.CeilToInt(newWidth / designRatio);
                    pixelRect = new Rect(0, (newHeight - screenHeight) / 2f, screenWidth, screenHeight);
                }
                UpdateCamerasRect();
            }
            #endregion
